...
async function run() {
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      default: (ans) => `com.${ans.projectName.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
    },
  ]);

  const { projectName, bundleIdentifier: newBundleIdentifier } = answers;
  ...
  filesToModifyGeneric.forEach(file => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/org\.reactjs\.native\.example\.\$\(PRODUCT_NAME:rfc1034identifier\)/g, newBundleIdentifier);
